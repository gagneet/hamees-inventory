...
Private Sub Worksheet_Change(ByVal Target As Range)
    ...
    ' Trigger only when the picker E5 changes
    If Intersect(Target, Me.Range("E5")) Is Nothing Then Exit Sub
    If Me.Range("B5").Value <> "Existing" Then Exit Sub
    
    Application.EnableEvents = False
    Application.ScreenUpdating = False
    
    ' Force recalc of formulas first
    Me.Range("B6:B16").Calculate
    
    ' Snapshot current formula results into static values so user can edit
    Dim addrs As Variant: addrs = Array("B7", "B8", "B9", "B10", "B11", "B13", "B14", "B15", "B16")
    Dim a As Variant
    For Each a In addrs
        Me.Range(CStr(a)).Value = Me.Range(CStr(a)).Value
    Next a
    
    ' Re-apply text format to phone cells
    Me.Range("B8").NumberFormat = "@"
    Me.Range("B14").NumberFormat = "@"
...
Private Sub Worksheet_SelectionChange(ByVal Target As Range)
    ' When user changes Mode to "New", restore formulas so they recalc to blanks
    If Target.Address = "$B$5" Then
        If Me.Range("B5").Value = "New" Then
            Call RestoreCustomerFormulas
        End If
    End If
    
End Sub

Public Sub RestoreCustomerFormulas()
    Application.EnableEvents = False
    Me.Range("B7").Formula = "=IF($B$5=""Existing"",IFERROR(XLOOKUP($B$6,Customers!$A:$A,Customers!$B:$B,""""),""""),"""")"
    Me.Range("B8").Formula = "=IF($B$5=""Existing"",IFERROR(XLOOKUP($B$6,Customers!$A:$A,Customers!$C:$C,""""),""""),"""")"
    Me.Range("B9").Formula = "=IF($B$5=""Existing"",IFERROR(XLOOKUP($B$6,Customers!$A:$A,Customers!$E:$E,""Yes""),""Yes""),""Yes"")"
    Me.Range("B10").Formula = "=IF($B$5=""Existing"",IFERROR(XLOOKUP($B$6,Customers!$A:$A,Customers!$F:$F,""""),""""),"""")"
    Me.Range("B11").Formula = "=IF($B$5=""Existing"",IFERROR(XLOOKUP($B$6,Customers!$A:$A,Customers!$H:$H,""Regular""),""Regular""),""Regular"")"
    Me.Range("B13").Formula = "=IF($B$5=""Existing"",IFERROR(XLOOKUP($B$6,Customers!$A:$A,Customers!$G:$G,""""),""""),"""")"
    Me.Range("B14").Formula = "=IF($B$5=""Existing"",IFERROR(XLOOKUP($B$6,Customers!$A:$A,Customers!$D:$D,""""),""""),"""")"
    Me.Range("B15").Formula = "=IF($B$5=""Existing"",IFERROR(XLOOKUP($B$6,Customers!$A:$A,Customers!$I:$I,""""),""""),"""")"
    Me.Range("B16").Formula = "=IF($B$5=""Existing"",IFERROR(XLOOKUP($B$6,Customers!$A:$A,Customers!$K:$K,""Yes""),""Yes""),""Yes"")"
    Application.EnableEvents = True
