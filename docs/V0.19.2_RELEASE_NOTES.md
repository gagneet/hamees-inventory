...
**Old Calculation:**
```
Is 75 < 20? No → Status: OK ❌ WRONG
```

**Actual Reality:**
```
Available = 75 - 69.35 = 5.65 meters
Is 5.65 < 10 (50% of 20)? Yes → Status: CRITICAL ✅ CORRECT
```
...
**Old Logic:**
- Low Stock: `available < (minimum × 1.1) AND > minimum` (10% buffer)
- Critical Stock: `available <= minimum`

**New Logic (Aligned with Dashboard):**
- Low Stock: `available < minimum AND available >= (minimum × 0.5)` (50-100% of minimum)
- Critical Stock: `available < (minimum × 0.5)` (below 50%)
...
### Critical Stock (Red 🔴)
- **Condition:** `available < (minimum × 0.5)`
...
### Low Stock (Amber 🟡)
- **Condition:** `available < minimum AND available >= (minimum × 0.5)`
...
#### ✅ Test 1: Critical Stock Calculation
```bash
...
#### ✅ Test 2: Clickable Chart - Critical Stock
```bash
...
#### ✅ Test 3: Clickable Chart - Low Stock
```bash
...
#### ✅ Test 4: Chart Segments vs Buttons
```bash
1. Click red segment in chart → Dialog opens
2. Close dialog
...
- ✅ Uses existing permission checks (`view_inventory`)
- ✅ Data filtered server-side based on user role
...
- ✅ Backward compatible with v0.19.1
...
3. Database has no items with available < minimum × 0.5
