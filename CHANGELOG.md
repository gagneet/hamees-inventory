...
- **Automatic reorder check and alerts** — an item needs reordering when its available stock plus what is already on order is at or below its minimum. The suggested quantity is the item's new *reorder quantity* setting, or enough to reach twice the minimum. The check runs after orders, stock edits and purchase-order changes, with alert generation, and on demand (Purchase Orders → Reorder suggestions). It keeps one reorder alert per item and, when Admin Settings → *Auto-reorder* is on, drafts one purchase order per supplier (priced from the supplier's current price) for approval — never approved automatically.
...
- An item on a draft purchase order counts as on order, so its reorder alert clears before the draft is approved. Reorder alerts for accessories show no item card on the alert page. The receive dialog's item list for legacy unlinked lines loads at most 500 items per type.
