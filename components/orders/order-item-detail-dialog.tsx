...
useEffect(() => {
  ...
  const handleSaveTailorNotes = async () => {
    ...
    try {
      // Create OrderHistory entry for tailor note
      const response = await fetch(`/api/orders/${orderItem.order.id}/tailor-notes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ note: tailorNotes }),
      })
    ...
    <>
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 mt-4">
          ...
          {/* Tailor's Work Notes - Historical with Audit Trail */}
          {canUpload && (
            <Card className="p-4 bg-green-50 border-green-200">
              ...
              <div className="bg-white p-3 rounded border-2 border-green-300">
                <p className="text-xs font-medium text-green-800 mb-2">Add New Note:</p>
                <textarea
                  className="w-full p-3 rounded border border-green-300 text-sm min-h-[80px] focus:outline-none focus:ring-2 focus:ring-green-500"
                  placeholder="Add observations, modifications, or special notes about this garment..."
                  value={tailorNotes}
